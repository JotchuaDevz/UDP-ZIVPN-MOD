...
obtener_usuarios() {
    ...
    if [[ -f "$DB_PATH" ]]; then
        sqlite3 "$DB_PATH" "SELECT password FROM users;" | paste -sd, -
    fi
}
...
EOF
    ...
    PORT=$(echo $UDP_PORT | tr -d ':')
    echo ""
    echo "════════════ URIs de conexion ════════════"
    while IFS= read -r passwd; do
        echo "udp-zivpn-labx://${SERVER_IP}:${PORT}?password=${passwd}"
...
EOF
    ...
    if [[ "$tabla_existe" == "users" ]]; then
        ...
        existe_db=$(sqlite3 "$USER_DB" "SELECT password FROM users WHERE password='$contrasena_defecto';")

        if [[ -z "$existe_db" ]]; then
            sqlite3 "$USER_DB" "INSERT INTO users (password) VALUES ('$contrasena_defecto');"
            if [[ $? -eq 0 ]]; then
                echo "Contraseña por defecto creada exitosamente."
            else
                echo "Error: No se pudo crear la contraseña por defecto."
            fi
        else
            echo "La contraseña por defecto ya existe."
        fi
    else
